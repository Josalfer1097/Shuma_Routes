...
export async function POST(req: NextRequest) {
  ...
  const { data, error } = await supabaseAdmin
    .from('user_profiles')
    .select('id, username, full_name, role, active, password_hash, driver_id, failed_attempts, locked_until')
  ...
  await logAction('login_success', 'session', data.id, data.username, 'driver',
    'Autenticación', { full_name: data.full_name, driver_id: data.driver_id, device: parseDevice(ua) }, ip, ua);

  const token = await signSession({
    sub: data.id, username: data.username, role: 'driver',
    fullName: data.full_name, driverId: data.driver_id,
  });

  const res = NextResponse.json({
    ok: true, role: 'driver', username: data.username,
    full_name: data.full_name, driver_id: data.driver_id,
  });
